//
//  Este programa MPI não contém um erro!
//  
//  Todos os programas em MPI devem possuir alguns comandos para
//  configurar o ambiente MPI, sendo eles:
//       - MPI_Init(): inicializa o MPI, deve ser invocado antes de qualquer outra chamada MPI;
//       - MPI_Finalize(): finaliza o MPI.
//
//  Para este programa funcionar corretamente, basta adicionar o comando
//  MPI_Finalize() ao final do código.  
//
//  Uso: mpirun -np 2 <prog>
//  
#include "mpi.h"
#include <stdio.h>
#include <stdlib.h>

int main(int argc,char *argv[])
{
   int numtasks, rank, dest, tag, source, rc;
   int inmsg, outmsg = 100;
   MPI_Status stat;

   MPI_Init(&argc, &argv);
   MPI_Comm_size(MPI_COMM_WORLD, &numtasks);
   MPI_Comm_rank(MPI_COMM_WORLD, &rank);

   printf("Processo %d iniciando...\n",rank);

   if (rank == 0) {
      dest = 1;
      source = dest;
      tag = 0;
      rc = MPI_Send(&outmsg, 1, MPI_INT, dest, tag, MPI_COMM_WORLD);
      printf("Enviei mensagem para processo %d...\n", dest);
      rc = MPI_Recv(&inmsg, 1, MPI_INT, source, tag, MPI_COMM_WORLD, &stat);
      printf("Recebi mensagem do processo %d...\n", source);
   }
   else if (rank == 1) {
      dest = 0;
      source = dest;
      tag = 0;
      rc = MPI_Recv(&inmsg, 1, MPI_INT, source, tag, MPI_COMM_WORLD, &stat);
      printf("Recebi mensagem do processo %d...\n", source);
      rc = MPI_Send(&outmsg, 1, MPI_INT, dest, tag, MPI_COMM_WORLD);
      printf("Enviei mensagem para processo %d...\n", dest);
   }

   MPI_Finalize();
}